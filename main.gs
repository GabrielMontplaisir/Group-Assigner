...
function randomAssign() {
  ...
  while (participantArray.length > 0) {
    const randomValue = getRandomInt(0, freeSessionsArray.length);
    const randomSession = freeSessionsArray[randomValue][0];
    const current = participants[participantArray[i][0]];
    
    const duplicate = hasSession(current, randomSession);
    if (duplicate) continue;

    if (sessions[randomSession].sessions[participantArray[i][1]].list.length < sessions[randomSession].maxParticipants) {
      current.assigned[participantArray[i][1]] = randomSession;
      sessions[randomSession].sessions[participantArray[i][1]].list.push(current.name);
      // console.log(`Assigning ${randomSession} to ${current.name} at session ${participantArray[i][1]}`);
      participantArray.splice(i, 1);
    }

    if (sessions[randomSession].sessions[freeSessionsArray[randomValue][1]].list.length >= sessions[randomSession].maxParticipants) {
      // console.log(`Removing ${freeSessionsArray[randomValue][0]} from queue.`);
      freeSessionsArray.splice(randomValue, 1);
      };
